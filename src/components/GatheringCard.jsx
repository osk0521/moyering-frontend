    ...
    <div className={styles.card} onClick={onClick}>
      ...
      <div className={styles.cardContent}>
        <div className={styles.cardTags}>
          ...
          <span className={`${styles.tag} ${styles.blue}`}><CiLocationOn />{gatherInfo.locName}</span>
        </div>
        <h3 className={styles.cardTitle}>{gatherInfo.title}</h3>
        <p className={`${styles.cardInfo} cardInfo_osk`} style={{ position: 'relative', bottom: '-20px' }}>
          <MdCalendarMonth /> {gatherInfo.meetingDate}
        </p>
      </div>
    </div>
  );
