...
const BannerManagement = () => {
  ...
  const handleEdit = (bannerId) => {
    ...
    const bannerToEdit = bannerList.find(
      (banner) => (banner.bannerId || banner.id) === bannerId
    );
  ...
  const toggleBannerStatus = async (bannerId, currentStatus) => {
    try {
      ...
      if (currentStatus === 1) {
        ...
        const visibleBanners = bannerList.filter(
          (banner) => banner.status === 1 || banner.status === null
        );

        if (visibleBanners.length >= 10) {
          alert(
            "보이는 배너는 최대 10개까지만 가능합니다.\n다른 배너를 먼저 숨긴 후 시도해주세요."
          );
          return;
        }
    ...
    try {
      ...
      if (editingBanner) {
        ...
        const response = await axios.put(
          `${url}/api/banner/${editingBanner.bannerId}`,
          formData,
          {
    ...
    <Layout>
      ...
      <div className="search-sectionHY">
        ...
        <div className="right-alignHY">
          <button
            className="btn-primary register-btnHY"
          ...
          <tbody>
            {bannerList.length === 0 ? (
              ...
              bannerList.map((banner, index) => {
                const visibleCount = bannerList.filter(
                  (b) => b.status === 1 || b.status === null
                ).length;
                    ...
                    <td>
                      <button
                        className={`status-toggle ${
                          isCurrentVisible ? "status-visible" : "status-hidden"
                        }`}
                        onClick={() =>
                          toggleBannerStatus(
                            banner.bannerId || banner.id,
                        ...
                        style={{
                          ...
                          cursor:
                            !canShow && !isCurrentVisible
                              ? "not-allowed"
                              : "pointer",
                        }}
                        title={
                          !canShow && !isCurrentVisible
                            ? "보이는 배너가 10개를 초과하여 비활성화됨"
                            : ""
                        }
                      >
                    ...
                    <td>
                      <div className="action-buttons">
                        <button
                          className="btn-edit"
                          onClick={() =>
                            handleEdit(banner.bannerId || banner.id)
                          }
                        >
                          수정
                        </button>
                        <button
                          ...
                          onClick={() =>
                            handleDelete(banner.bannerId || banner.id)
                          }
                        >
                          삭제
                        </button>
